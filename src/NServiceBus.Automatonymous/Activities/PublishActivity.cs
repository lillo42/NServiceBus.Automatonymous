...
namespace NServiceBus.Automatonymous.Activities
{
    ...
    public class PublishActivity<TInstance, TMessage> : Activity<TInstance>
        where TInstance : class, IContainSagaData
        where TMessage : class
    {
        private readonly Action<BehaviorContext<TInstance>, PublishOptions>? _configureOptions;
        ...
        /// <summary>
        /// Initialize new instance of <see cref="PublishActivity{TInstance,TMessage}"/>.
        /// </summary>
        ...
        public PublishActivity(Func<BehaviorContext<TInstance>, TMessage> messageFactory, 
            Action<BehaviorContext<TInstance>, PublishOptions>? configureOptions)
        {
        ...
        /// <param name="configureOptions">The <see cref="Action{T1,T2}"/> to configure <see cref="SendOptions"/>.</param>
        public PublishActivity(Func<BehaviorContext<TInstance>, Task<TMessage>> asyncMessageFactory, 
            Action<BehaviorContext<TInstance>, PublishOptions>? configureOptions)
        {
        ...
        private async  Task Execute(BehaviorContext<TInstance> context)
        {
            ...
            await context.GetPayload<IMessageHandlerContext>().Publish(message, options).ConfigureAwait(false);
        }
    }
    ...
    /// <typeparam name="TMessage">The message type.</typeparam>
    public class PublishActivity<TInstance, TData, TMessage> : Activity<TInstance, TData>
        where TInstance : class, IContainSagaData
        where TMessage : class
    {
        private readonly Action<BehaviorContext<TInstance, TData>, PublishOptions>? _configureOptions;
        ...
        /// <param name="messageFactory">The sync factory of <typeparamref name="TInstance"/>.</param>
        /// <param name="configureOptions">The <see cref="Action{T1,T2}"/> to configure <see cref="SendOptions"/>.</param>
        public PublishActivity(Func<BehaviorContext<TInstance, TData>, TMessage> messageFactory, 
            Action<BehaviorContext<TInstance, TData>, PublishOptions>? configureOptions)
        {
            _messageFactory = messageFactory ?? throw new ArgumentNullException(nameof(messageFactory));
            _configureOptions = configureOptions;
        }
        ...
        /// <param name="asyncMessageFactory">The async factory of <typeparamref name="TInstance"/>.</param>
        /// <param name="configureOptions">The <see cref="Action{T1,T2}"/> to configure <see cref="SendOptions"/>.</param>
        public PublishActivity(Func<BehaviorContext<TInstance, TData>, Task<TMessage>> asyncMessageFactory, 
            Action<BehaviorContext<TInstance, TData>, PublishOptions>? configureOptions)
        {
            _asyncMessageFactory = asyncMessageFactory ?? throw new ArgumentNullException(nameof(asyncMessageFactory));
            _configureOptions = configureOptions;
